...
VTraceReturns = collections.namedtuple("VTraceReturns", "vs pg_advantages mask")
...
def from_logits(
    ...
    vtrace_returns = from_importance_weights(
        target_policy_log_probs=target_policy_logits,
        behavior_policy_log_probs=behavior_policy_logits,
        log_rhos=log_rhos,
        discounts=discounts,
        rewards=rewards,
        values=values,
...
def from_importance_weights(
    ...
    with torch.no_grad():
        kl_div = (behavior_policy_log_probs.exp() * (behavior_policy_log_probs - target_policy_log_probs)).sum(-1).unsqueeze(-1)
        ...
        pg_advantages = clipped_pg_rhos * (rewards + discounts * vs_t_plus_1 - values) * mask
